...
#Count occurance of 1s in first position
#if more 1 count > Data.Length/2, gamma bit is 1. Else 0
#once all bits counted, convert new binary number to decimal
...
foreach($position in (0..($ReportData[0].Length-1))){
    Write-Host "Checking position $position"
    $1count = 0
...
$gammaInt = [Convert]::ToInt32($gammaBinary,2)
#2^12 = 4096
$epsilonInt = $gammaInt -bxor 4095
